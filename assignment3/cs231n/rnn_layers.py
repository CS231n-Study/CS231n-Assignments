...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # ! two sum of multiplication => one matrix multiplication
    W = np.vstack((Wh, Wx))
    X = np.hstack((prev_h, x))

    # ! Activation function
    next_h = np.tanh(X @ W + b)
    cache = (W, X, b)
...
def rnn_step_backward(dnext_h, cache):
    ...
    D_and_H, H = W.shape
    N, _ = X.shape # D_tmp = D + H

    dX = np.zeros_like(X)
    dW = np.zeros_like(W)
    # ! next_h = np.tanh(X @ W + b)
    # ? dtanh(N, H) = dnext_h(N, H) * (1 - linear_comb(N, H)**2)
    affine = X @ W + b
    dtanh = dnext_h * (1 - np.tanh(affine)**2)

    # ! linear_comb = X @ W + b
    # ? dX(N, D+H) = dtanh(N, H) @ W.T(H, D+H)
    # ? dW(D+H, H) = X.T(D+H, N) @ dtanh(N, H)
    dX = dtanh @ W.T
    dW = X.T @ dtanh
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(1, T)):
        dx_tmp, dprev_h_t, dWx_tmp, dWh_tmp, db_tmp = rnn_step_backward(dnext_h, cache[t])
        dx[:, t, :] += dx_tmp
        dWx += dWx_tmp
        dWh += dWh_tmp
        db += db_tmp
...
def word_embedding_backward(dout, cache):
    ...
    # ? arr: indices로 인덱싱 되는 타겟 배열
    # ? indices: arr를 인덱싱하는 데에 사용하는 index 배열(or matrix)
    # ? values: arr에 누적시킬 값
    np.add.at(dW, x, dout) # x가 indices로서 사용됨. 
