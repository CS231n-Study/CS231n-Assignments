...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            if i == 0:
                dim_arr[i] = (input_dim, hidden_dims[i])
            elif i == self.num_layers - 1:
                dim_arr[i] = (hidden_dims[i-1], num_classes)
            else:
                dim_arr[i] = (hidden_dims[i-1], hidden_dims[i])
        ...
        for i in range(self.num_layers):
            ...
            if (normalization is not None) and (i + 1 < self.num_layers):
    ...
    def loss(self, X, y=None):
        ...
        fn_affine_bn_relu_forward = None
        fn_affine_bn_relu_backward = None
        if self.normalization is not None:
            fn_affine_bn_relu_forward = affine_bn_relu_forward \
                if self.normalization == 'batchnorm' else affine_ln_relu_forward
            fn_affine_bn_relu_backward = affine_bn_relu_backward \
                if self.normalization == 'batchnorm' else affine_ln_relu_backward
        ...
        for i in range(1, self.num_layers):
            ...
            if self.normalization is not None:
                ...
                out, cache = fn_affine_bn_relu_forward(out, W, b, gamma, beta, self.bn_params[i-1])
            else:
        ...
        # ! last output: score matrices
        # ? last layer should not be normalized -> only affine forward
        ...
        for i in reversed(range(1, self.num_layers)):
            ...
            if self.normalization is not None:
                cache = cache_arr.pop(-1)
                dout, grads[f'W{i}'], grads[f'b{i}'], grads[f'gamma{i}'], grads[f'beta{i}'] = fn_affine_bn_relu_backward(dout, cache)
            else:
                cache = cache_arr.pop(-1)
